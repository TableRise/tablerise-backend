...
export default class UpdateUserOperation {
    ...
    constructor({ usersSchema, updateUserService, schemaValidator, logger }: UpdateUserOperationContract) {
    ...
    public async execute({ userId, payload }: UpdateUserPayload): Promise<RegisterUserResponse> {
        ...
        const { user, userDetails } = await this._updateUserService.update({ userId, payload });
