...
import { CheckCharactersPayload, UpdateMatchPlayersPayload } from 'src/types/api/campaigns/http/payload';
...
export default class UpdateMatchPlayersService {
    ...
    private async _checkForCharacters({ userId, characterId }: CheckCharactersPayload): Promise<void> {
    ...
    async updateMatchPlayers({
        ...
        if (operation === 'add') {
            ...
            await this._checkForCharacters({ userId, characterId } as CheckCharactersPayload);
