...
export default function generateDungeonsAndDragonsFaker({
    ...
    entityId,
    entity
}: { count: number, entityId: string | undefined, entity: string | undefined }): any[] {
    const entityArray: Array<Internacional<Armor>> = [];

    for (let index = 0; index <= count; index += 1) {
        entityArray.push(dungeonsAndDragonsFunctions[entity as keyof typeof dungeonsAndDragonsFunctions]({ entityId } as { entityId: string }));
    }
