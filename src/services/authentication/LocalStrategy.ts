...
passport.use(
    new LocalStrategy({
        ...
        if (isDataInvalid) return done(new HttpRequestErrors({
        ...
        if (!user.length) return done(new HttpRequestErrors({
        ...
        if (!isPasswordValid) return done(new HttpRequestErrors({
