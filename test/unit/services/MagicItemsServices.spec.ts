...
describe('Services :: MagicItemsServices', () => {
  ...
  describe('When the recover a magic item by ID service is called', () => {
    beforeAll(() => {
      jest.spyOn(MagicItemsModelMock, 'findOne').mockResolvedValueOnce(magicItemMockInstance)
  ...
  describe('When service for update a magic item is called', () => {
    ...
    beforeAll(() => {
      jest.spyOn(MagicItemsModelMock, 'update').mockResolvedValueOnce(magicItemMockUpdateInstance)
        .mockResolvedValue(null);
    });

    it('should return correct data with updated values', async () => {
      const responseTest = await MagicItemsServicesMock.update(magicItemMockID, magicItemMockPayload as Internacional<MagicItem>);
      expect(responseTest).toBe(magicItemMockUpdateInstance);
    });

    it('should throw an error when payload is incorrect', async () => {
      try {
        await MagicItemsServicesMock.update(magicItemMockID, magicItemMockPayloadWrong as Internacional<MagicItem>);
      } catch (error) {
        const err = error as Error;
