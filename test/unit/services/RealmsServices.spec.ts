...
describe('Services :: RealmsServices', () => {
  ...
  describe('When the recover a realm by ID service is called', () => {
    beforeAll(() => {
      jest.spyOn(RealmsModelMock, 'findOne').mockResolvedValueOnce(realmsMockInstance)
  ...
  describe('When service for update a realm is called', () => {
    ...
    beforeAll(() => {
      jest.spyOn(RealmsModelMock, 'update').mockResolvedValueOnce(realmMockUpdateInstance)
        .mockResolvedValue(null);
    });

    it('should return correct data with updated values', async () => {
      const responseTest = await RealmsServicesMock.update(realmMockID, realmsMockPayload as Internacional<Realm>);
