...
describe('Get RPG gods from database', () => {
    ...
    describe('When request one rpg god', () => {
        it('should return a god instance', async () => {
            ...
            const { body } = await request(app).get(`/gods/${documentId}`).expect(HttpStatusCode.OK);
        ...
        it('should fail when ID NotFound', async () => {
            const { body } = await request(app).get(`/gods/${generateNewMongoID()}`).expect(HttpStatusCode.NOT_FOUND);
