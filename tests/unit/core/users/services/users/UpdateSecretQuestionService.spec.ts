    ...
    context('#update', () => {
        context('When update an user secret question with success', () => {
            ...
            it('should return the correct result', async () => {
                const userDetails = await updateSecretQuestionService.update({ userId: '123', payload });

                expect(userDetails.secretQuestion?.question).to.be.equal(payload.new.question);
                expect(userDetails.secretQuestion?.answer).to.be.equal(payload.new.answer);
            });
        });

        context(
            'When update an user secret question fail ',
            () => {
                before(() => {
                    userDetails = DomainDataFaker.generateUserDetailsJSON()[0];
