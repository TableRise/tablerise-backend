    ...
    context('When called', () => {
        ...
        context('should delete users with more than 15 days with status wait_to_delete', () => {
            ...
            it('should return the correct result', async () => {
                const deleUserScheduledTaskTest = await deleteUserCronJob(usersRepository, usersDetailsRepository);
                ...
                expect(daysDifferenceSpy).to.have.been.calledWith(Date.parse(userToExclude.updatedAt), WAIT_TO_EXCLUSION_PERIOD);
                expect(daysDifferenceSpy).to.have.been.calledWith(Date.parse(user.updatedAt), WAIT_TO_EXCLUSION_PERIOD);
