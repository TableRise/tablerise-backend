...
describe('Interface :: Dungeons&dragons5e :: Presentation :: Feats :: FeatsRoutes', () => {
    let featsRoutes: FeatsRoutes;
    let featsController: Record<string, sinon.SinonStub>;
    ...
    beforeEach(() => {
        ...
        featsRoutes = new FeatsRoutes({
            featsController: featsController as unknown as FeatsController,
            verifyIdMiddleware,
        });
    });
    ...
    context('When all the routes are correctly implemented', () => {
        it('should return the current route table and parse the availability query', () => {
            ...
            expect(routes[0]).to.deep.equal({ basePath: '/system/dnd5e/feats' });
            expect(routes[1].controller).to.equal(featsController.getAll);
            expect(routes[2].controller).to.equal(featsController.getDisabled);
            expect(routes[3].controller).to.equal(featsController.get);
            expect(routes[4].controller).to.equal(featsController.toggleAvailability);
