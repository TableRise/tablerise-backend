...
describe('When a user details cover is uploaded', () => {
    ...
    it('should save the cover object on user details', async () => {
        await requester()
        ...
        expect(savedUserDetails.cover).to.be.an('object');
    ...
    it('should reject uploads for another authenticated user id', async () => {
        await requester()
            .patch(`/users/${anotherUserId}/update/cover`)
            .attach('image', filePath)
